...
void Game::initializeLocations() {
    ...
    Location* floor4Corridor = new Location(12, "ЭТАЖ 4 - КОРИДОР",
        ...
        "Существо заходит туда, дверь за ним с грохотом закрывается дальше перед\n"
...
void Game::processOption(LocationOption* option) {
    ...
    if (option->id == 1 && player->getLocation() == 0) {
       ...
       if (!player->getInventory().hasItem("Пропуск")) {
        ...
        if (!player->getInventory().hasItem("Кусок плоти")) {
        ...
        else{std::cout<<"Смотря на это, вы ощущаете странное чувство дежавю.\n"
        "Словно бы это уже происходило с вами когда-то. Словно бы эта девушка уже умирала,\n"
        "но что-то не отпустило её из этого мира. Но сейчас на полу туалета лежит тело, из\n"
        "раздробленного черепа вытекает кровь и серое вещество\n";}
    }
        ...
        if (finalAnswer == "Человек") {
            ...
            std::cout << "Кто вы? Как вы здесь оказались? А главное - зачем?\n"
                "На эти вопросы вы не знаете ответа. Но вы знаете - главное:\n"
                "вы успешно защитились. Увы, вы не получите диплом. Вам он уже\n"
                "не зачем. Теперь вы - часть этого места. И вас больше ничего не волнует\n" << std::endl;
            ending = "КОНЦОВКА 4. Вы защитились\n";
        }
